            ...
            fetch(config.urlValidar, {
                ...
                .then((data) => {
                    ...
                    if (data.status === "success") {
                        ...
                        let rankingHtml =
                            '<div class="mt-3"><h5 class="text-warning mb-3">🏆 Top Jugadores</h5><ul class="list-group list-group-flush text-start" style="font-size: 0.9rem;">';
                        ...
                        if (data.rankingGlobal && data.rankingGlobal.length > 0) {
                            data.rankingGlobal.slice(0, 5).forEach((puesto, index) => {
                                ...
                                <li class="list-group-item bg-transparent border-secondary d-flex justify-content-between align-items-center py-2" style="${bg}">
                                    <div>
                                        <span class="fw-bold ${color} me-2">#${index + 1
                                    }</span>
                                        <span class="${esUsuarioActual
                                        ? "text-info fw-bold"
                                        : "text-light"
                                    }">${escapeHtml(
                                        puesto.nombre_jugador
                                    )}</span>
                                    </div>
                                    <span class="badge bg-dark text-light border border-secondary">⏱ ${puesto.tiempo_segundos
                                    }s</span>
                                </li>`;
                            });
                        } else {
                            rankingHtml +=
                                '<li class="list-group-item bg-transparent text-white-50 text-center">Sin datos aún</li>';
                        }
                        rankingHtml += "</ul></div>";
                ...
                const item = `
                    <li class="list-group-item list-group-item-dark-custom d-flex justify-content-between align-items-center"> 
                        <div class="text-truncate" style="max-width: 65%;">
                            <span class="fw-bold ${index == 0 ? "text-warning" : "text-white"
                    }">#${index + 1}</span>
                            ${esUsuarioActual
                        ? `<strong class="text-info ms-1">${escapeHtml(
                            puesto.nombre_jugador
                        )}</strong>`
                        : `<span class="text-white ms-1">${escapeHtml(
                            puesto.nombre_jugador
                        )}</span>`
                    }
                            <div style="font-size: 0.75rem;" class="text-light ms-4">${new Date(
                        puesto.fecha
                    ).toLocaleDateString("es-ES")}</div>
                        </div>
                        <span class="badge bg-warning text-dark rounded-pill">⏱ ${puesto.tiempo_segundos
                    }s</span>
                    </li>`;
                ...
                const item = `
                    <li class="list-group-item list-group-item-dark-custom d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                            <span class="fw-bold text-info me-3">#${index + 1
                    }</span>
                            <div>
                                <span class="badge ${nivelClase}">${puesto.nivel.charAt(0).toUpperCase() + puesto.nivel.slice(1)
                    }</span>
                                <small class="d-block text-white-50 mt-1">${new Date(
                        puesto.fecha
                    ).toLocaleDateString("es-ES")}</small>
                            </div>
                        </div>
                        <span class="badge bg-info text-dark rounded-pill">⏱&nbsp;${puesto.tiempo_segundos
                    }s</span>
                    </li>`;
